/*
 APP: Advance Weather App

 Features we'll need to create
 =============================================
    1. currentWeather - Gets the current weather
    2. forecast - Gets 5 day forecast

These are all the functions we'll need to build
================================================
    1. getWeatherData() - Runs both features then updates the DOM by running updateDom().
...
const getWeatherData = async () => {
  ...
  try {
    // Create a const that stores the user input from the searchbar or defaults back to 'Delhi' if left blank
    const city = searchInput.value || 'Delhi'

    // Create 2 promises that call the APIs and pass in the city name
    // If the user haven't typed anything, use Delhi as default
